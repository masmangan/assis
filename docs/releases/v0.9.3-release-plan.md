...
ASSIS v0.9.3 is released **as is**, with the following acknowledgement:
...
```bash
ls assis-cli/target/assis-cli-0.9.3.jar
```

Run the client and check version:
```bash
javar -jar assis-cli/target/assis-cli-0.9.3.jar --version
```
- Version v0.9.3 expected
...
If needed, review commit history since v0.9.2 to confirm scope:
